<x-app-layout>
    ...
    <div class="container mx-auto px-4 py-6">
        ...
        <div class="relative">
            ...
            <div id="dropdownMenu" class="hidden absolute mt-1 w-48 bg-white shadow-lg rounded z-10 ring-opacity-5">
                <a href="{{ route('reports.product_sales')}}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-200">Productos más vendidos</a>
                <a href="{{ route('reports.least-sales')}}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-200">Productos menos vendidos</a>
        ...
        <div class="flex mt-8 gap-5">
            <div class="flex-1">
                <h2 class="text-lg font-bold mb-4">Tabla de Productos Peores Ingresos</h2>
                <table class="min-w-full bg-white rounded-lg overflow-hidden shadow-lg">
                    <thead class="bg-gray-100">
                        <tr>
                            <th class="border px-8 py-4">Nombre del Producto</th>
                            <th class="border px-8 py-4">Cantidad Vendida</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($leastRevenueProducts as $product)
                            <tr>
                                <td class="border px-8 py-4">{{ $product['name'] }}</td>
                                <td class="border px-8 py-4">{{ $product['revenue'] }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            ...
            <div class="flex-1">
                <h2 class="text-lg font-bold mb-4">Gráfico de Productos Que Menos Generan</h2>
                <canvas id="leastRevenueProducts" width="400" height="250"></canvas>
            </div>
        </div>
    </div>
    ...
    <script>
        var leastRevenueProductsCtx = document.getElementById('leastRevenueProducts').getContext('2d');
        var leastRevenueProductsData = @json($leastRevenueProducts);

        new Chart(leastRevenueProductsCtx, {
            type: 'bar',
            data: {
                labels: leastRevenueProductsData.map(data => data.name),
                datasets: [{
                    label: 'Unidades vendidas',
                    data: leastRevenueProductsData.map(data => data.revenue),
                    backgroundColor: leastRevenueProductsData.map(() => getRandomColor()), // Genera un color para cada barra
                    borderColor: leastRevenueProductsData.map(() => getRandomColor()), // Opcional: puedes usar un solo color para todos los bordes si prefieres
                    borderWidth: 1
                }]
            },
            options: {
