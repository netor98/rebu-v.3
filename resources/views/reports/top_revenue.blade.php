<x-app-layout>
    ...
    <div class="container mx-auto px-4 py-6">
        ...
        <div class="flex mt-8 gap-5">
            <div class="flex-1">
                <h2 class="text-lg font-bold mb-4">Tabla de Productos Más Vendidos</h2>
                <table class="min-w-full bg-white rounded-lg overflow-hidden shadow-lg">
                    ...
                    <tbody>
                        @foreach ($topRevenueCustomers as $product)
                            <tr>
                                <td class="border px-8 py-4">{{ $product['name'] }}</td>
                                <td class="border px-8 py-4">{{ $product['total_revenue'] }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            ...
            <div class="flex-1">
                <h2 class="text-lg font-bold mb-4">Gráfico de Productos Más Vendidos</h2>
                <canvas id="topRevenueCustomersChart" width="400" height="250"></canvas>
            </div>
        </div>
    </div>
    ...
    <script>
        var topRevenueCustomersCtx = document.getElementById('topRevenueCustomersChart').getContext('2d');
        var topRevenueCustomersData = @json($topRevenueCustomers);

        new Chart(topRevenueCustomersCtx, {
            type: 'bar',
            data: {
                labels: topRevenueCustomersData.map(data => data.name),
                datasets: [{
                    label: 'Unidades vendidas',
                    data: topRevenueCustomersData.map(data => data.total_revenue),
                    backgroundColor: topRevenueCustomersData.map(() => getRandomColor()), // Genera un color para cada barra
                    borderColor: topRevenueCustomersData.map(() => getRandomColor()), // Opcional: puedes usar un solo color para todos los bordes si prefieres
                    borderWidth: 1
                }]
            },
            options: {
