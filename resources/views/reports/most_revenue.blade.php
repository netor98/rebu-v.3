<x-app-layout>
    ...
    <div class="container mx-auto px-4 py-6">
        ...
        <div class="flex mt-8 gap-5">
            <div class="flex-1">
                <h2 class="text-lg font-bold mb-4">Tabla de Productos Mejores Ingresos</h2>
            ...
            <div class="flex-1">
                <h2 class="text-lg font-bold mb-4">Gráfico de Productos Que Más Generan</h2>
                <canvas id="mostRevenueProducts" width="400" height="250"></canvas>
            </div>
        </div>
    </div>
    ...
    <script>
        var mostRevenueProductsCtx = document.getElementById('mostRevenueProducts').getContext('2d');
        var mostRevenueProductsData = @json($mostRevenueProducts);

        new Chart(mostRevenueProductsCtx, {
            type: 'bar',
            data: {
                labels: mostRevenueProductsData.map(data => data.name),
                datasets: [{
                    label: 'Unidades vendidas',
                    data: mostRevenueProductsData.map(data => data.revenue),
                    backgroundColor: mostRevenueProductsData.map(() => getRandomColor()), // Genera un color para cada barra
                    borderColor: mostRevenueProductsData.map(() => getRandomColor()), // Opcional: puedes usar un solo color para todos los bordes si prefieres
                    borderWidth: 1
                }]
            },
            options: {
