<x-app-layout>
    ...
    <div class="container mx-auto px-4 py-6">
        ...
        <div class="flex mt-8 gap-5">
            <div class="flex-1">
                <h2 class="text-lg font-bold mb-4">Tabla de Productos Menos Vendidos</h2>
                    ...
                    <tbody>
                        @foreach ($leastSoldProducts as $product)
                            <tr>
                                ...
                                <td class="border px-8 py-4">{{ $product['count'] }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            ...
            <div class="flex-1">
                <h2 class="text-lg font-bold mb-4">Gráfico de Productos Menos Vendidos</h2>
                <canvas id="leastSoldProducts" width="400" height="250"></canvas>
            </div>
        </div>
    </div>
    ...
    <script>
        var leastSoldCtx = document.getElementById('leastSoldProducts').getContext('2d');
        var leastSoldData = @json($leastSoldProducts);

        new Chart(leastSoldCtx, {
            type: 'bar',
            data: {
                labels: leastSoldData.map(data => data.name),
                datasets: [{
                    label: 'Unidades vendidas',
                    data: leastSoldData.map(data => data.count),
                    backgroundColor: leastSoldData.map(() => getRandomColor()), // Genera un color para cada barra
                    borderColor: leastSoldData.map(() => getRandomColor()), // Opcional: puedes usar un solo color para todos los bordes si prefieres
                    borderWidth: 1
                }]
            },
            options: {
