<x-app-layout>
    ...
    <div class="container mx-auto px-4 py-6">
        ...
        <div class="flex mt-8 gap-5">
            ...
            <div class="flex-1">
                ...
                <canvas id="mostSoldChart" width="400" height="250"></canvas>
            </div>
        </div>
    </div>
    ...
    <script>
        var mostSoldCtx = document.getElementById('mostSoldChart').getContext('2d');
        var mostSoldData = @json($mostSoldProducts);

        var dataArray = Object.values(mostSoldData);
        ...
        new Chart(mostSoldCtx, {
            type: 'bar',
            data: {
                labels: dataArray.map(data => data.name),
                datasets: [{
                    label: 'Unidades vendidas',
                    data: dataArray.map(data => data.count),
                    backgroundColor: dataArray.map(() => getRandomColor()), // Genera un color para cada barra
                    borderColor: dataArray.map(() => getRandomColor()), // Opcional: puedes usar un solo color para todos los bordes si prefieres
                    borderWidth: 1
                }]
            },
            options: {
